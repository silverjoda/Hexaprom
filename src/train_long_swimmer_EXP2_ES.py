...
def f(w):
    ...
    while not done:
        ...
        t0 = np.tanh(np.matmul(o0, w[0: n1_a].reshape((8, 3))))
        t0 = np.tanh(np.matmul(t0, w[n1_a: n1_a + n1_b].reshape((3, 1))))

        # -------------

        # Oscillator 1
        o1 = list(env_obs[[1, 2, 3, 11, 12, 13]]) + prev_torques[0:3]
        t1 = np.tanh(np.matmul(o1, w[n1:n1 + n2_a].reshape((9, 3))))
        t1 = mult * np.tanh(np.matmul(t1, w[n1 + n2_a:N_weights].reshape((3, 1))))

        # Oscillator 2
        o2 = list(env_obs[[2, 3, 4, 12, 13, 14]]) + prev_torques[1:4]
        t2 = np.tanh(np.matmul(o2, w[n1:n1 + n2_a].reshape((9, 3))))
        t2 = mult * np.tanh(np.matmul(t2, w[n1 + n2_a:N_weights].reshape((3, 1))))

        # Oscillator 3
        o3 = list(env_obs[[3, 4, 5, 13, 14, 15]]) + prev_torques[2:5]
        t3 = np.tanh(np.matmul(o3, w[n1:n1 + n2_a].reshape((9, 3))))
        t3 = mult * np.tanh(np.matmul(t3, w[n1 + n2_a:N_weights].reshape((3, 1))))

        # Oscillator 4
        o4 = list(env_obs[[4, 5, 6, 13, 14, 15]]) + prev_torques[2:5]
        t4 = np.tanh(np.matmul(o4, w[n1:n1 + n2_a].reshape((9, 3))))
        t4 = mult * np.tanh(np.matmul(t4, w[n1 + n2_a:N_weights].reshape((3, 1))))

        # Oscillator 5
        o5 = list(env_obs[[5, 6, 7, 14, 15, 16]]) + prev_torques[3:6]
        t5 = np.tanh(np.matmul(o5, w[n1:n1 + n2_a].reshape((9, 3))))
        t5 = mult * np.tanh(np.matmul(t5, w[n1 + n2_a:N_weights].reshape((3, 1))))

        # Oscillator 6
        o6 = list(env_obs[[6, 7]]) + [0] + list(env_obs[[15, 16]]) + [0] + prev_torques[4:6] + [0]
        t6 = np.tanh(np.matmul(o6, w[n1:n1 + n2_a].reshape((9, 3))))
        t6 = mult * np.tanh(np.matmul(t6, w[n1 + n2_a:N_weights].reshape((3, 1))))
