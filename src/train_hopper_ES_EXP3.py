...
def f(w):
    ...
    while not done:
        ...
        # Oscillator 0
        o0 = list(env_obs[[0, 1, 5, 6, 7]]) + prev_torques[0:1]
        t0 = np.tanh(np.matmul(o0, w[0:n1_a].reshape((6, 3))))
        t0, s0 = mult * np.tanh(np.matmul(t0, w[n1_a:n1].reshape((3, 2))))

        # Oscillator 1
        o1 = list(env_obs[[2,3,4]]) + [s0] + prev_torques[1:2]
        t1 = np.tanh(np.matmul(o1, w[n1:n1+n2_a].reshape((5, 3))))
        t1, s1 = mult * np.tanh(np.matmul(t1, w[n1+n2_a:n1 + n2].reshape((3, 2))))

        # Oscillator 2
        o2 = list(env_obs[[2,3,4]]) + [s0,s1] + prev_torques[2:3]
        t2 = np.tanh(np.matmul(o2, w[n1 + n2:n1 + n2 + n3_a].reshape((6, 3))))
