...
def f(w):
    ...
    while not done:
        ...
        # Perform env_obs normalization
        rstat.push(env_obs)
        env_obs = (env_obs - rstat.mean) / rstat.std

        # Master node
        obs = list(env_obs[[0, 1, 8, 9, 10]]) + list(rnn_states['h_h1']) + list(rnn_states['h_h2'])
        h_m_s = np.matmul(wdist.get_w('m_w', w).T, rnn_states['h_m']) + np.matmul(wdist.get_w('m_u', w).T, nfobs(obs)) + wdist.get_w('m_b', w)
        h_m = np.tanh(h_m_s)
        y_m = np.matmul(h_m, wdist.get_w('m_v', w)) + wdist.get_w('m_c', w)

        # h1 node
        obs = list(env_obs[2:3]) + list(y_m[0:n_hidden_osc]) + list(rnn_states['h_k1'])
        h_h1_s = np.matmul(wdist.get_w('h_w', w).T, rnn_states['h_h1']) + np.matmul(wdist.get_w('h_u', w).T, nfobs(obs)) + wdist.get_w('h_b', w)
        h_h1 = np.tanh(h_h1_s)
        ...
        h_h2_s = np.matmul(wdist.get_w('h_w', w).T, rnn_states['h_h2']) + np.matmul(wdist.get_w('h_u', w).T,nfobs(obs)) + wdist.get_w('h_b', w)
        h_h2 = np.tanh(h_h2_s)
        y_h2 = np.matmul(wdist.get_w('h_v', w).T, h_h2) + wdist.get_w('h_c', w)

        # k1 node
        obs = list(env_obs[3:4]) + list(rnn_states['h_h1']) + list(rnn_states['h_f1'])
        h_k1_s = np.matmul(wdist.get_w('k_w', w).T, rnn_states['h_k1']) + np.matmul(wdist.get_w('k_u', w).T, nfobs(obs)) + wdist.get_w('k_b', w)
        h_k1 = np.tanh(h_k1_s)
        y_k1 = np.matmul(wdist.get_w('k_v', w).T, h_k1) + wdist.get_w('k_c', w)

        # k2 node
        obs = list(env_obs[6:7]) + list(rnn_states['h_h2']) + list(rnn_states['h_f2'])
        h_k2_s = np.matmul(wdist.get_w('k_w', w).T, rnn_states['h_k2']) + np.matmul(wdist.get_w('k_u', w).T,nfobs(obs)) + wdist.get_w('k_b', w)
        h_k2 = np.tanh(h_k2_s)
        y_k2 = np.matmul(wdist.get_w('k_v', w).T, h_k2) + wdist.get_w('k_c', w)

        # f1 node
        obs = list(env_obs[4:5]) + list(rnn_states['h_k1'])
        h_f1_s = np.matmul(wdist.get_w('f_w', w).T, rnn_states['h_f1']) + np.matmul(wdist.get_w('f_u', w).T, nfobs(obs)) + wdist.get_w('f_b', w)
        h_f1 = np.tanh(h_f1_s)
        y_f1 = np.matmul(wdist.get_w('f_v', w).T, h_f1) + wdist.get_w('f_c', w)

        # f2 node
        obs = list(env_obs[7:8]) + list(rnn_states['h_k2'])
        h_f2_s = np.matmul(wdist.get_w('f_w', w).T, rnn_states['h_f2']) + np.matmul(wdist.get_w('f_u', w).T, nfobs(obs)) + wdist.get_w('f_b', w)
        h_f2 = np.tanh(h_f2_s)
        ...
        #haps = [h_m_s, h_h1_s, h_k1_s, h_f1_s, h_h2_s, h_k2_s, h_f2_s]
        #print(haps)

        action = [y_h1, y_k1, y_f1, y_h2, y_k2, y_f2]
        #action = [a + np.random.randn() * 0.01 for a in action]
